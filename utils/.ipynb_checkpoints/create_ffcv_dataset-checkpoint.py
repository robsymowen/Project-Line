'''
    ...
    datasets (even in1k), have some freakishly large images. By defult we enforce the 
    longest side constraint with a center crop.
    
    Args:
        dataset: name of dataset (loaded by name from `datasets` folder)
        
    Example Use:
    python create_ffcv_dataset.py \
        --cfg.dataset=imagenet1k \
        --cfg.split=val \
        --cfg.data_dir=/path/to/imagenet1k/root/folder/ \
        --cfg.write_path=/path/to/ffcv_files/imagenet1k_val_jpg_q100_s256_lmax512_crop.ffcv \
        --cfg.write_mode=jpg \
...
from helpers import ffcv_writers as writers
...
Section('cfg', 'arguments to give the writer').params(
    ...
    writer_type=Param(And(str,OneOf(writer_types)), 'Which writer to use', default='image_label_path')
)
...
class ImageNet1k(datasets.ImageNet):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any, Any, Any]:
        ...
        return img, target, rel_path
    
def imagenet1k(*args, **kwargs):
    return ImageNet1k(*args, **kwargs)
...
def get_dataset(dataset, split, data_dir, subset):
    my_dataset = imagenet1k(root=data_dir, split=split)
